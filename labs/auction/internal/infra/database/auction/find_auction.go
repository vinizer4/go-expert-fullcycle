...
func (ar *AuctionRepository) FindAuctionById(
	...
	if err := ar.Collection.FindOne(ctx, filter).Decode(&auctionEntityMongo); err != nil {
		logger.Error(
			fmt.Sprintf("Error trying to find auction by id = %s", id), err)
...
func (ar *AuctionRepository) FindAllAuctions(
	...
	if err != nil {
		logger.Error("Error trying to find all auctions", err)
		return nil, internal_error.NewInternalServerError("Error trying to find all auctions")
	}
	...
	if err = cursor.All(ctx, &auctionsEntityMongo); err != nil {
		logger.Error("Error trying to decode all auctions", err)
		return nil, internal_error.NewInternalServerError("Error trying to decode all auctions")
	}
